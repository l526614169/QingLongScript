...
class TelecomLotter:
    ...
    def find_price(self):
        ...
        if data["code"] == 0:
            ...
            compare_date = lambda date: date.split("-")[1] == str(
                (datetime.now() + timedelta(hours=8 - int(strftime("%z")[2]))).month)
            month_price = [f'{info["win_time"]}: {info["title"]}' for info in all_price_list if
        ...
        for i in body:
            if time.strftime('%Y-%m-%d') in i['start_time']:
                if i not in all_list:
                    ...
                    # print('\n')
                    msg_str += '今日开播时间：' + i['start_time'] + ' 直播间名称：' + i[
