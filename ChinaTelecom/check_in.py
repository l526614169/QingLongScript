...
account_list = get_environ("TELECOM_PHONE").split(",")
...
def add(txt):
    return a2b_hex(''.join(['2d2d2d2d2d424547494e205055424c4943204b45592d2d2d2d2d0a4d4947664d',
                            '413047435371475349623344514542415155414134474e4144434269514b4267',
                            '51432b756747354138635a334671554b44774d3537474d34696f360a4a476353',
                            ...
                            '726a432f72696477687739666657324c4277766b456e446b6b4b4b5269326c69',
                            '574949744466744a564269574f6831376f36676662506f4e72574f52630a4164',
        ...
        try:
            ...
            if jsont == None:
                ...
                if jsont["data"]["code"] == 1:
                    ...
                    msgg.append(
                        f'签到结果:{jsont["data"]["msg"]},获得{jsont["data"]["coin"]}金豆,已签到{jsont["data"]["totalDay"]}天')
                else:
            ...
            for task in self.task_list:
                if "翻牌抽好礼" in task["title"] or "查看我的订单" in task["title"] or "查看我的云盘" in task["title"]:
                    ...
                    if jsont["data"]["code"] == 0:
                        print(
                            f'每日任务:{task["title"].split(" ")[0]},{jsont["resoultMsg"]},{task["title"].split(" ", 1)[1]}')
